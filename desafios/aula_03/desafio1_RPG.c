...
Celula buscaInimigoMaisProximo(Celula atual) {
    ...
    // procura o primeiro inimigo na lista
    Celula curr = atual->prox;

    while (curr != atual) {

        if (curr->personagem.tipo != atual->personagem.tipo) {
            return curr;
        }
...
void removeDaListaCircular(ListaCircular lista, Celula alvo) {
    ...
    // procura o nó anterior ao alvo
    while (curr->prox != alvo &&
           curr->prox != lista->inicio) {
